...
import wandb
from opt import OPT
from common_gpt import TEMPLATE_CONFIG
from common import Imbalanced_Metric, pmi, task_to_keys, LABEL2ID_CONFIG, LABEL_CONVERT, constrainScoreByWholeExact, counter, ApiCallLimitError
...
def parse_args():
    ...
    parser.add_argument("--model_name_or_path", default='gpt2-xl', choices=['gpt2-xl', 'llama3'], type=str)
...
def main():
    ...
    if args.model_name_or_path == 'llama3':
        tokenizer = AutoTokenizer.from_pretrained(pretrained_model_name_or_path, use_fast=not args.use_slow_tokenizer, cache_dir=model_cache_dir)
    else:
    ...
    def preprocess_function(examples):
        ...
        template_base = template_cfg

        texts = []
        template = [template_base] * len(examples[sentence1_key])
        offset = 1000
        #prompt = tokenizer.decode(list(range(offset, offset+args.n_prompt_tokens)))
        prompt = list(range(offset, offset+args.n_prompt_tokens))
        prompt_attn = torch.ones_like(torch.tensor(list(range(offset, offset+args.n_prompt_tokens)))).tolist()
        if sentence2_key:
            for tuple_ in list(zip(examples[sentence1_key], template, examples[sentence2_key])):
                ...
                texts.append(new_sent_1 + " . " + new_sent_2 + tuple_[1])
                #texts.append(prompt + " . " + new_sent_1 + " . " + new_sent_2 + tuple_[1])
        ...
        result['input_ids'] = [prompt+inputs for inputs in result['input_ids']]
        result['attention_mask'] = [prompt_attn+attn for attn in result['attention_mask']]
    ...
    def run_zo(args, batch, z_t, prompts_probs):
        ...
        for k in range(int(args.sample_size/2)):
            ...
            new_batch = update_p0_gpt(args, batch, prompts_probs, indices_list)
            
            noise = torch.normal(mean=0.0, std=1.0, size=z_t_tmp.shape).to(args.device)
            #noise = torch.normal(mean=0.0, std=1.0, size=z_t.shape).repeat(len(batch['input_ids']), 1).to(args.device)
    ...
    #prompts_probs = prompts_alpha
    #prompts_probs = torch.FloatTensor([[1 / args.prompt_search_space] * args.prompt_search_space] * args.n_prompt_tokens)
    ...
    try:
        while True:
            for step, batch in enumerate(train_dataloader):
                ...
                # constrainScoreByWholeExact(prompts_alpha)
                # prompts_probs = prompts_alpha

                #new_batch = update_p0_gpt(args, batch, prompts_probs)
